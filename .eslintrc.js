module.exports = {
  ...
  rules: {
     ...
     * Здесь "comments": 120 указывает максимальную длину комментариев в 120
     * символов, а "code": 120 указывает максимальную длину строки кода в 120
     * символов.
     */
     ...
     * 1. Пустая строка всегда добавляется после объявлений переменных (const, let, var) перед любым следующим
     * выражением.
     ...
     * Правило `arrow-body-style` требует, чтобы тело стрелочной функции было обрамлено фигурными скобками только тогда,
     * когда это необходимо. "as-needed" указывает, что фигурные скобки не обязательны, если функция состоит из одного
     * выражения.
     */
    
    'prefer-arrow-callback': 'off',
    /**  https://eslint.org/docs/latest/rules/prefer-arrow-callback
     * Правило `prefer-arrow-callback` предлагает использовать стрелочные функции в качестве коллбэков вместо
     * обычных функций. "off" отключает это правило, что позволяет использовать как стрелочные функции, так и
     * обычные функции в зависимости от предпочтений разработчика.
     */
     ...
     * "avoidEscape": true позволяет использовать двойные кавычки, если строка
     * содержит одинарную кавычку и избегает экранирования.
     ...
     * Например, если вы импортируете файл из текущего каталога, использование
     * "../" перед именем файла может быть излишним.
